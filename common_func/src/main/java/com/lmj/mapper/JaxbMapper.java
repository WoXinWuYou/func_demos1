...
import com.lmj.utils.Reflections;
...
public class JaxbMapper {
	 ...
	 * @param xml
	 * @param clazz
	 * @param childClazz 泛型子类型
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public static <T> T fromXml(String xml, T t) {
		return (T) fromXml(xml, t.getClass());
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> T fromXml(String xml, Class... classesToBeBound) {
		try {
			StringReader reader = new StringReader(xml);
			...
			if (jaxbContext == null) {
				try {
					...
					String classNamesJoint = null;
					for (Class class1 : classesToBeBound) {
						if(classNamesJoint == null) {
							classNamesJoint = class1.getName();
						}else {
							classNamesJoint = ","+class1.getName();
						}
					}
					throw new HttpMessageConversionException("Could not instantiate JAXBContext for class [" + classNamesJoint
							+ "]: " + ex.getMessage(), ex);
				}
			}
